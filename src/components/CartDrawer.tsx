...
export function CartDrawer({ isOpen, onClose }: CartDrawerProps) {
  ...
  const handleCheckout = () => {
    ...
    let message = 'Olá! Gostaria de finalizar a seguinte compra:\n\n';
    
    cartItems.forEach(item => {
      const price = item.discount ? item.price * (1 - item.discount / 100) : item.price;
      message += `${item.quantity}x ${item.name} - ${formatCurrency(price)}\n`;
    });
    ...
    <>
      ...
      >
        <div className="flex items-center justify-between p-6 border-b border-zinc-200 dark:border-zinc-800">
          <h2 className="text-lg font-black uppercase tracking-tight text-zinc-900 dark:text-white">Sua Sacola</h2>
          <button 
            onClick={onClose}
        ...
        {cartItems.length === 0 ? (
          <div className="flex-1 overflow-y-auto p-6 flex flex-col items-center justify-center text-center">
            ...
            <h3 className="text-lg font-bold text-zinc-900 dark:text-white mb-2 uppercase tracking-wide">Sacola Vazia</h3>
          ...
          <>
            ...
            <div className="p-6 border-t border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-950">
              <div className="flex items-center justify-between mb-4">
                <span className="text-sm font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wide">Total</span>
